...
BROKER_PORT = -1
LOG_FILE = f"../_logs/amqp_ponte_cenario_{CENARIO}_{time.strftime('%Y-%m-%d-%H-%M-%S')}.csv"
...
def callback(ch, method, properties, body):
    ...
    channel.basic_publish(exchange='', routing_key='destinatario', body=payload)
    contador += 1

channel.basic_consume(queue='ponte', on_message_callback=callback, auto_ack=True)
